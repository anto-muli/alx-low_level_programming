...
/**
 * _strncmp - compare 2 strs
 * @s1: the 1st string
 * @s2: the 2nd string
 * @n: the max # of bytes to compare
 *
 * Return: 0 if the 1st n bytes of s1 and s2 are =, otherwise non-zero
 */
...
/**
 * _close - closes file descriptor and prints an error msg when failed
...
/**
 * _read - read a file's contente and prints an error message if there is failure
 * @fd: the read file descriptor
 * @buf: the buffer to write to
 * @count: the # of bytes to read
 */
...
/**
 * elf_class - prints ELF class number
 * @buffer: the ELF header number
 *
 * Return: identifies the bit mode either 32 or 64
 */
 ...
 * @buffer: the ELF header #
 *
 * Return: 1 if its  big endian or 0 if not
 */
...
/**
 * elf_osabi - print ELF OS/ABI no.
 * @buffer: outputs the ELF header
 */
...
/**
 * elf_abivers - outputs the ELF ABI version
 * @buffer: the ELF header
 */
...
/**
 * elf_type - shows the ELF type
 * @buffer: the ELF header
 * @big_endian:  hihglights the endianness (big endian if non-zero)
 */
...
/**
 * elf_entry - print the  address of the entry point
 * @buffer: string containing the entry of the  point address
 * @bit_mode: outputs the bit mode either 32 or 64
 * @big_endian: shows the endianness (big endian if non-zero)
 */
...
/**
 * main - copies the  content of a  file contents to another file
 * @argc: the args count
